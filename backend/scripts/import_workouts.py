...
# Mapping for categories to match model conventions (if needed)
CATEGORY_MAP = {
...
def run():
    print("Starting import of workouts...")
    count = 0
    for w in data["workouts"]:
        ...
        # Default properties
        gender = "male" # Defaulting to male based on typical splits, but could be 'Unisex'
        # Actually usually workouts are unisex. The model has 'gender' field. 
        # Let's set to 'male' as default for split routines often marketed that way, 
        # or update logic to be smarter. 
        # 'Glutes Focus' suggests female often, but let's stick to a safe default or randomize?
        # Better: Set to "male" for heavy lifting, "female" for others? 
        # Or just 'male' as per likely request context or just alternating.
        # User didn't specify. I'll default to "male" as it's a common default in such apps, or "Unisex" if valid.
        # Model definition: gender = models.CharField(max_length=10) # male, female
        # It says "male, female" in comment.
        ...
        # Check if exists
        if not Workout.objects.filter(name=name).exists():
            ...
            print(f"Skipped existing workout: {name}")
            
    print(f"Import complete. Added {count} workouts.")
